...
This file provides guidance to Claude Code when reviewing writing in this folder.

## Purpose

This folder contains writing for review using the Writing Mentor Framework. The primary task is:
...
- `guidelines.md` - Writing guidelines and requirements
- `criteria.md` - Evaluation criteria with point allocations
- `submissions/` - Writing submissions (PDF, DOCX, XLSX, etc.)
- `similarity_reports/` - Similarity reports (optional)
- `skills/feedback/` - Framework instructions, scripts, and references
...
```bash
python skills/feedback/scripts/check_dependencies.py
```
...
python skills/feedback/scripts/extract_submission_text.py --input submissions --out feedback_extracted

# Render Excel charts for visual review and formula validation
python skills/feedback/scripts/render_xlsx_quicklook.py --input submissions --out feedback_rendered
```

Then enumerate submissions by parsing filenames. Group files by writer (using the filename prefix before the first underscore). For writers with multiple submissions, identify the final version (highest version number).
...
INSTRUCTIONS: Read and follow the detailed workflow in skills/feedback/SKILL.md - it contains the two-tier output format, data validation procedures, and examples.

REFERENCES (READ ALL BEFORE REVIEWING):
- Writing guidelines: guidelines.md
- Evaluation criteria: criteria.md
- Economical writing principles: skills/feedback/references/economical_writing_principles.md (REQUIRED)
- Domain concepts: skills/feedback/references/domain_concepts.md (if available - enhances assumption validation)
...
- If multiple versions exist, review only the FINAL version (highest version number)
- If prior feedback/{username}.md exists, include revision assessment

SIMILARITY REPORT: {similarity_info}
...
TWO-TIER OUTPUT:
- SECTION A (Reviewer Notes): Technical audit, exact issues, data validation findings, evidence trail. FOR REVIEWER ONLY.
...
```bash
python skills/feedback/scripts/render_xlsx_excel.py --input submissions --out feedback_rendered
```
...
4. (Recommended) Add `skills/feedback/references/domain_concepts.md` with domain concepts to enhance assumption validation
